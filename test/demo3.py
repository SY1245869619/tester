...
class HTML_REPORT:
    def __init__(self, html_title, pie_theme, test_info, report_table_title, report_cases):
        ...
        if isinstance(report_cases, dict) and len(report_cases) > 0:
            ...
            self.pie_sum_number = {'right_sum': len(report_cases['right_cases']),
                                   ...
                                   'untreated_sum': len(report_cases['untreaded_cases'])}
        else:
    ...
    def genHtmlReport(self, html_template):
        ...
        for data_key in self.report_cases.keys():
            ...
            '''
        export_lable_datas = REPORT_TMPL_Lable.format(len(report_cases['all_cases']), len(report_cases['right_cases']),
                                                      len(report_cases['error_cases']),
        ...
        html = template(html_template, report_title=self.html_title, theme=self.pie_theme, \
                        ...
                        right_sum=self.pie_sum_number['right_sum'], error_sum=self.pie_sum_number['error_sum'],
...
'''
    report_cases = {'all_cases': [ \
        ['理财', '1', '登录', 'lo144444444444444444444444444444444444444444444444444444444444444gin', 'post', 'Pass', '详细',
         case_detail], \
        ...
        'untreaded_cases': [
            ['WEB理财', '3', '测试', 'login', 'po6666666666666666666666666666666666666666666666666666st', '/', '详细',
             case_detail]]
    }
